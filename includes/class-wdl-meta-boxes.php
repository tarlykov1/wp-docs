...
class WDL_Meta_Boxes {
    ...
    public function render($post){ wp_nonce_field('wdl_save_meta','wdl_meta_nonce'); $fields=['file_id','file_url','version','updated_date','owner','doc_number','expiry_date','card_description','pdf_viewer','show_download','important','new','manual_order']; foreach($fields as $f){ $$f=get_post_meta($post->ID,'_wdl_'.$f,true);} $document_category_taxonomy = self::DOCUMENT_CATEGORY_TAXONOMY; $document_categories=get_terms(array('taxonomy'=>$document_category_taxonomy,'hide_empty'=>false)); $selected_categories=wp_get_object_terms($post->ID,$document_category_taxonomy,array('fields'=>'ids')); include WDL_PLUGIN_DIR.'templates/admin-metabox.php'; }
    public function save($post_id){ if (wp_is_post_autosave($post_id) || wp_is_post_revision($post_id)) return; if (!isset($_POST['wdl_meta_nonce'])||!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['wdl_meta_nonce'])),'wdl_save_meta')) return; if (!current_user_can('edit_post',$post_id)) return; $text=['version','updated_date','owner','doc_number','expiry_date']; foreach($text as $k){ if(isset($_POST['wdl_'.$k])) update_post_meta($post_id,'_wdl_'.$k,sanitize_text_field(wp_unslash($_POST['wdl_'.$k]))); }
        ...
        if (isset($_POST['wdl_document_category'])) {
            ...
            if ($term_id > 0) {
                $term = get_term($term_id, self::DOCUMENT_CATEGORY_TAXONOMY);
                if ($term instanceof WP_Term && self::DOCUMENT_CATEGORY_TAXONOMY === $term->taxonomy) {
                    wp_set_object_terms($post_id, array($term_id), self::DOCUMENT_CATEGORY_TAXONOMY, false);
                }
            } else { wp_set_object_terms($post_id, array(), self::DOCUMENT_CATEGORY_TAXONOMY, false); }
        }
    }
}

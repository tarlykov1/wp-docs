...
while (have_posts()) :
    ...
    if (! $has_file && $file_url) {
        ...
        if ($baseurl && $basedir && str_starts_with((string) $file_url, $baseurl)) {
            $relative = ltrim(substr((string) $file_url, strlen($baseurl)), '/');
            $candidate = trailingslashit($basedir) . $relative;
            if ($relative !== '' && file_exists($candidate)) {
                $has_file = true;
            }
        } else {
            // Для внешних URL проверить наличие файла на диске невозможно.
            $has_file = true;
        }
    }
    ...
    <article class="wpdl-document-page wpdl-single-document wdl-single">
        <div class="wpdl-document-layout wpdl-single-document-layout">
            <?php if (has_post_thumbnail($post_id)) : ?>
                <div class="wpdl-document-thumbnail wpdl-single-document-thumbnail" style="max-width: <?php echo esc_attr($thumb_width); ?>px; <?php if ('auto' !== $thumb_height) : ?>height: <?php echo esc_attr($thumb_height); ?>px;<?php endif; ?>">
            ...
            <div class="wpdl-document-content wpdl-single-document-content">
                ...
                <?php elseif (get_the_content()) : ?>
                    <section class="wpdl-document-description wpdl-single-document-description">
                        <h2>Описание</h2>
                        <div><?php echo wp_kses_post(wpautop(get_the_content())); ?></div>
                    </section>
                <?php endif; ?>
                ...
                <?php if ($file_url && $show_download && $has_file) : ?>
                ...
                $excluded = array(
                    '_wdl_file_id', '_wdl_file_url', '_wdl_version', '_wdl_updated_date', '_wdl_owner', '_wdl_doc_number',
                    '_wdl_expiry_date', '_wdl_card_description', '_wdl_show_download', '_wdl_pdf_viewer', '_wdl_important',
                    '_wdl_new', '_wdl_manual_order', '_edit_lock', '_edit_last', '_thumbnail_id',
                );
                ...
                foreach ($all_meta as $meta_key => $values) {
                    ...
                    $label = trim(str_replace('_', ' ', str_replace('_wdl_', '', $meta_key)));
                    $extra_meta_rows[$label] = sanitize_text_field((string) $value);
                }
                    ...
                    <section class="wpdl-single-document-fields">
                        ...
                        <dl class="wpdl-document-meta">
                            <?php foreach ($extra_meta_rows as $label => $value) : ?>
                                <div class="wpdl-document-meta-row">
                                    <dt><?php echo esc_html(mb_convert_case($label, MB_CASE_TITLE, 'UTF-8')); ?></dt>
                                    <dd><?php echo esc_html($value); ?></dd>
                                </div>
                            <?php endforeach; ?>
                        </dl>
                ...
                <?php if (get_the_content()) : ?><div class="wdl-single-body"><?php the_content(); ?></div><?php endif; ?>
            </div>
        </div>
    </article>
